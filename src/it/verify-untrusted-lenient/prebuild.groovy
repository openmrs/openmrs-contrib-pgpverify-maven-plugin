...
// Negative path with the opt-out flag on: an ephemeral key signs the dependency and
// its real public key is supplied so the key resolves, but the keys file pins a
// DIFFERENT fingerprint. Verification must reject it as untrusted and fail the build
// even though failOnMissingSignature=false - a trust failure is not a "missing" one.
...
Key-Type: RSA
Key-Length: 2048
Name-Real: OpenMRS PGPVerify Untrusted-Lenient IT
Name-Email: untrusted-lenient-it@example.org
Expire-Date: 0
%commit
"""
...
// Purge any copy a previous run cached in the isolated local repository, so the
// resolver does not serve a stale .asc against this run's freshly exported key.
File cached = new File(localRepositoryPath, "org/openmrs/ittest/untrusted-lenient-dep")
if (cached.exists()) {
	cached.deleteDir()
}
File repoDir = new File(fixtures, "repo/org/openmrs/ittest/untrusted-lenient-dep/1.0.0")
repoDir.mkdirs()
File jar = new File(repoDir, "untrusted-lenient-dep-1.0.0.jar")
new ZipOutputStream(new FileOutputStream(jar)).withCloseable { zos ->
	zos.putNextEntry(new ZipEntry("META-INF/MANIFEST.MF"))
	zos.write("Manifest-Version: 1.0\n".getBytes("UTF-8"))
	zos.closeEntry()
}
new File(repoDir, "untrusted-lenient-dep-1.0.0.pom").text = """<project>
	<modelVersion>4.0.0</modelVersion>
	<groupId>org.openmrs.ittest</groupId>
	<artifactId>untrusted-lenient-dep</artifactId>
	<version>1.0.0</version>
	<packaging>jar</packaging>
</project>
"""
gpg(["--local-user", fingerprint, "--armor", "--detach-sign",
     "--output", new File(repoDir, "untrusted-lenient-dep-1.0.0.jar.asc").absolutePath, jar.absolutePath])
